...
INPUT_CSV = ROOT / "data/bureaux-vote/cabestany-polling-stations-import.csv"
SCHEMA_SQL = ROOT / "data/bureaux-vote/polling-stations-import-schema.sql"
...
def build_sql(reset_scope: bool) -> str:
    ...
    if reset_scope:
        statements.append(
            "delete from import_campaign.polling_stations_cabestany where commune_code = '66028';"
        )

    statements.extend(
        [
            "\\copy import_campaign.polling_stations_cabestany ("
            "commune_name, commune_code, polling_station_code, polling_station_number, "
            "polling_station_ref, place_name, address, is_centralizer, source_pdf, geometry_type"
            f") from '{input_path}' csv header;",
            "select commune_code, count(*) as station_count, count(distinct place_name) as distinct_places "
            "from import_campaign.polling_stations_cabestany where commune_code = '66028' group by commune_code;",
        ]
    )
    return "\n".join(statements) + "\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Importe les bureaux de vote enrichis de Cabestany dans PostgreSQL."
    )
    ...
    parser.add_argument("--reset", action="store_true", help="supprime d'abord les lignes de Cabestany")
...
def main() -> int:
    ...
    print(f"Polling station rows: {row_count}")
